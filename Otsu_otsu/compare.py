...
otsu_in_path=".\\otsu_images\\"
otsu_out_path=".\\comparisions_otsu\\"
...
for file in files:
    ...
    otsu_in=cv2.imread(otsu_in_path+file)

    otsu_tesseract=cv2.imread(otsu_in_path+file)
    otsu_tesseract=tesseract_box(otsu_tesseract,(11,237,255))

    image_stack=numpy.hstack((original_LR,interpulated,otsu_in,original_HR,otsu_tesseract,ocr_original))
    final_image=numpy.vstack((image_stack,label_image))

    cv2.imwrite(otsu_out_path+file,final_image)
    print("Done :: "+file)