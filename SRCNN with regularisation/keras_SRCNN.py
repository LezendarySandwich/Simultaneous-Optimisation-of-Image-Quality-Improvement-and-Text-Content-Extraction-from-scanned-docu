...
path_training_Y='C:\\Users\\dell\\Desktop\\super resolution data\\train\\HR\\'
...
path_validate_Y='C:\\Users\\dell\\Desktop\\super resolution data\\validate\\HR\\'
...
model.add(Conv2D(32, kernel_size=5, padding='same', activation='relu', kernel_regularizer=regularizers.l2(0.0001), use_bias=True))
model.add(Conv2D(1, kernel_size=5, padding='same', activation='linear',kernel_regularizer=regularizers.l2(0.0001), use_bias=True))
adam=Adam(lr=0.0001)
...
model.fit(training_data,steps_per_epoch=(num_training_examples//batch_size),epochs=epochs,verbose=1,validation_data=validate_data,validation_steps=(num_validate_examples//batch_size))
